...
public class WorldBuilder : MonoBehaviour {
	...
	public GameObject readWld (string filepath) {
		this.filepath = filepath;
		world = Instantiate (Resources.Load ("Empty")) as GameObject;
		IO io = new IO();
		if (!io.Load (filepath))
			return null;
		while (true) {
	...
	void addWall (string[] args) {
		...
		floor.transform.localScale = new Vector3 (Vector2.Distance(start, end),0.1f,0.01f);
		floor.transform.position = new Vector3 ((end.x+start.x)/2,0.05f,(end.y+start.y)/2);
		floor.transform.rotation = Quaternion.Euler (0,Mathf.Atan2(end.y-start.y,end.x-start.x)/Mathf.PI*180,0);
